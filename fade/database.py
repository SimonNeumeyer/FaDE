...
import json
import functools
from .persistence import dir_data
from .util import Singleton, Constants, uuid
from .graph import Graph


class Database(metaclass=Singleton):
    def __init__(self):
        self.db = sql.connect(os.path.join(dir_data, Constants.database_name))
        if not self._table_exists(Constants.table_versioning):
            self._create_table_versioning()
        self.versions = dict(self.db.execute(f"SELECT * FROM {Constants.table_versioning}").fetchall())
        self._backup()
    ...
    def _table_exists(self, tablename):
        return self.db.execute("SELECT name FROM sqlite_master WHERE type='table' AND name=(?)", (tablename,)).fetchone() != None
    ...
    def get_samples (self, tablename, number_samples=0, filter:dict={}, accept_less=False):
        assert self._table_exists(tablename), f"Table {tablename} does not exist"
        schema = self._get_schema(tablename)
        filter_sql = "" if len(filter) == 0 else "WHERE " + " AND ".join(
            [f"{key} = '{filter[key]}'" if not isinstance(filter[key], list) else
             f"{key} IN {self._toSqlList(filter[key])}" for key in filter])
        ...
        assert(len(result)==number_samples or number_samples==0 or accept_less), f"Only {len(result)} samples available"
        return [dict(zip(schema, row)) for row in result]

    def _get_schema (self, tablename):
        assert self._table_exists(tablename), f"Table {tablename} does not exist"
        return [column[1] for column in self.db.execute(f"PRAGMA table_info({tablename})").fetchall()]

    def update_schema (self, tablename, columns): #columns = {columnname: datatype}
        assert self._table_exists(tablename), f"Table {tablename} does not exist"
        self.versions[tablename] = self.versions[tablename] + 1 if tablename in self.versions else 0
        for col in columns.items():
            self.db.execute(f"ALTER TABLE {tablename} ADD COLUMN {col[0]} {col[1]}")
        self.db.commit()

    def update_schema_delete_column (self, tablename, columns): #columns = list of column names
        assert self._table_exists(tablename), f"Table {tablename} does not exist"
        self.versions[tablename] = self.versions[tablename] + 1 if tablename in self.versions else 0
        for col in columns:
    ...
    def insert_results(self, results):
        ...
        self.db.executemany((f"INSERT INTO {Constants.table_results} VALUES ({self.versions[Constants.table_results]},"
                            """
                            :graph_id,
                            :settings_id,
                            :stage_index,
                            ...
                            :model_accuracies,
                            :model_loss,
                            :model_losses)"""), results)
        self.db.commit()

    def insert_graphs(self, graphs):
        tablename = Constants.table_graphs
        self._insert(tablename)
        self.db.executemany((f"INSERT INTO {tablename} VALUES ("
                             """
                            :id,
                            :technical,
                            :number_vertices,
                            :number_operations)"""), graphs)
        self.db.commit()

    def insert_settings(self, settings):
        self._insert(Constants.table_settings)
        self.db.execute((f"INSERT INTO {Constants.table_settings} VALUES ({self.versions[Constants.table_settings]},"
                            ...
                            f":{Constants.organizational_experiment_group})"), settings)
        self.db.commit()

    def insert_versioning(self, versioning):
        self.db.executemany(f"INSERT INTO {Constants.table_versioning} VALUES (?,?)", versioning)
        self.db.commit()

    def _create_table_results (self):
        self.db.execute((f"CREATE TABLE {Constants.table_results} ("
                        """
                        version TEXT,
                        graph_id TEXT,
                        settings_id TEXT,
    ...
    def _create_table_settings (self):
        self.db.execute((f"CREATE TABLE {Constants.table_settings} ("
    ...
    def _create_table_graphs (self):
        self.db.execute(f"CREATE TABLE {Constants.table_graphs} ("
                        """
                        id TEXT,
                        technical TEXT,
    ...
    def _one_time_init_graphs (self):
        ...
        graphs_total = [{'id': uuid(time=False), 'technical': g.get_technical(), 'number_vertices': g.get_number_vertices()}
                        for g in graphs_total]
        self.insert_graphs(graphs_total)

    def _create_table_versioning (self):
        self.db.execute((f"CREATE TABLE {Constants.table_versioning} ("
    ...
    def _input (self):
        self.insert_settings({Constants.settings_id: uuid(),
...
if __name__ == '__main__':
    settings = {Constants.settings_id: uuid(),
                Constants.settings_comment: "",
                Constants.dataset: "MNIST",
    ...
    for lr in [0.05, 1]:
        for w in [17, 43, 73]:
            for n in [0, 1]:
                settings.update({Constants.settings_id: uuid(),
                                 ...
                                 Constants.organizational_experiment_group: 'gridsearch' +str(n)})
