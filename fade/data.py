...
class MyDataset(AbstractDataset):
    def __init__(self, size=100, number_features=20, overwrite=False):
        ...
        self.path = os.path.join(Constants.path_data_base, "MyDataset")
    ...
    def create_data(self):
        path_dir = os.path.join(self.path, self.file_input)
        if os.path.isfile(path_dir):
            return

        inputs = numpy.array(
            [
                numpy.around(numpy.random.rand(self.number_features))
    ...
    def calculate_output(self, i):
        input_size = len(i)
        criteria = int(
            numpy.sum(i[int(input_size / 4) : int(input_size / 2)])
            >= numpy.sum(i[int(input_size / 2) : int(3 * input_size / 4)])
        )
        return numpy.array([criteria, 1 - criteria], dtype=numpy.float32)

    def read_data(self):
        return self.read_file(self.path / self.file_input), self.read_file(
            self.path / self.file_output
        )
    ...
    def get_train_test_loader(self, batchSize):
        ...
        loader_train = DataLoader(
            ...
            sampler=SubsetRandomSampler(
                range(0, int(self.train_test_ratio * self.size))
            ),
        ...
        loader_test = DataLoader(
            self,
            sampler=SubsetRandomSampler(
                range(int(self.train_test_ratio * self.size), self.size)
            ),
            batch_size=batchSize,
            num_workers=self.num_workers,
        )
...
class MNIST(AbstractDataset):
    def __init__(self, one_hot_target=True):
        ...
        self.dataset = ConcatDataset(
            [self._create(train=True), self._create(train=False)]
        )
        self.ratio_distinct_total = 0.0
    ...
    def get_subset(self, ratio, distinct):
        ...
        self.ratio_distinct_total = (
            self.ratio_distinct_total + ratio if distinct else self.ratio_distinct_total
        )
        return torch.utils.data.Subset(
            self.dataset,
            range(
                math.ceil(ratio_start * len(self)),
                math.ceil((ratio_start + ratio) * len(self)),
            ),
        )
