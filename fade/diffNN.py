...
class DiffNN(nn.ModuleList):
    ...
    def get_alpha (self, graphNN_id):
        alpha_list = numpy.array(util.get_numeric(self._calculate_alphas(sampling=False)))[numpy.array(list(self.graphNNs.keys())) == graphNN_id]
    ...
    def _calculate_alphas(self, sampling, train_mode=True):
        if not train_mode:
            return torch.zeros_like(self.alphas).scatter_(0, torch.argmax(self.alphas, dim=0), 1.)
        if sampling:
    ...
    def _reduce(self, alphas, tensors):
        ...
        if self.settings[Constants.model_operation] == Constants.OPERATION_CONV:
            ...
            reduced = reduced.view(reduced.shape[0], reduced.shape[1], int(width), int(width))
    ...
    def forward(self, x, train_mode):
        alphas = self._calculate_alphas(sampling=self.settings[Constants.darts_sampling], train_mode=train_mode)
...
class GraphNN(nn.Module):
    def __init__(self, graph, settings, number_channels=None, kernel_size=None, stride=None, padding=None, shared_edges=None):
        super(GraphNN, self).__init__()
        self.id = util.uuid()
        self.graph = graph
        self.settings = settings
        assert self.settings[Constants.model_operation] in [Constants.OPERATION_CONV, Constants.OPERATION_LINEAR]
        if self.settings[Constants.model_operation] == Constants.OPERATION_CONV:
            assert(not (number_channels is None or kernel_size is None or stride is None or padding is None)), "Conv parameters not provided"
    ...
    def _get_operation (self):
        if self.settings[Constants.model_operation] == Constants.OPERATION_CONV:
            module = nn.Conv2d(self.number_channels, self.number_channels, kernel_size=self.kernel_size, stride=self.stride, padding=self.padding)
        else:
            module = nn.Linear(self.settings[Constants.mlp_layer_width], self.settings[Constants.mlp_layer_width])
    ...
    def forward(self, x):
        ...
        for v in self.graph.get_ordered_nodes():
            inputs = [self.edgeNNs[self._stringify_edge((p, v))](outputs[p]) for p in self.graph.get_predecessors(v)]
