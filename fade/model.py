...
class AbstractModel(nn.Module):
    def __init__ (self, settings, graphs, dataset_number_channels, dataset_number_features, dataset_number_classes):
    ...
    def _interface(self):
        ...
        self.graphNNs = functools.reduce(lambda a, b: {**a, **b},
                                         [{g: {self.key_stage_id: s.get_id(), self.key_module: s.get_graphNNs()[g]}} for s in self.stages for g in s.get_graphNNs()])
    ...
    def get_stage_index (self, graphNN_id):
        ...
        return self.stage_dict[self.graphNNs[graphNN_id][self.key_stage_id]].get_stage_index()
    ...
    def get_parameters(self, alphas):
        ...
        for name, param in self.named_parameters():
            if self.is_alpha_parameter(name) and alphas or not self.is_alpha_parameter(name) and not alphas:
                params.append(param)
        return params
    ...
    def get_alpha (self, graphNN_id):
        ...
        assert graphNN_id in self.graphNNs
        return self.stage_dict[self.graphNNs[graphNN_id][self.key_stage_id]].get_alpha(graphNN_id)

    def set_alpha_update (self, alphaUpdate): #Deprecated, proper DARTS implementation should not need it
...
class MLP(AbstractModel):  # Multi Layer Perceptron
    def __init__(self, **kwargs):
        ...
        self.features = kwargs["dataset_number_channels"] * kwargs["dataset_number_features"]
        assert self.settings[Constants.model_activation] in [Constants.ACTIVATION_RELU], f"Activation {self.activation} not supported"
        self.linear_0 = nn.Sequential(nn.Linear(self.features, self.settings[Constants.mlp_layer_width]), nn.ReLU())
        self.linear_1 = nn.Sequential(nn.Linear(self.settings[Constants.mlp_layer_width], kwargs["dataset_number_classes"]))
        self._create_stages()
        self._interface()
...
class ConvolutionalModel(AbstractModel):
    ...
    def _create_stages(self):
        assert(math.isclose(round(self.number_features), self.number_features)), f"Number features {self.dataset_number_features} no square"
        assert(self.pooling_type in [Constants.POOLING_MAX, Constants.POOLING_AVG]), f"Pooling {self.pooling_operation} not supported"
        assert(self.kernel_size in [3,5,7]), f"Kernel size {self.kernel_size} not supported"
        assert(self.final_number_channels) >= 1
        self.pooling_kernel_size = 2
        self.stride = 1
        base, self.padding, channel_counts = self._calculate_pooling(self.final_number_channels, self.number_stages, self.number_features)
        ...
        if self.number_stages == 1:
            self.linear_1 = nn.Linear(self.final_number_channels * 8 * 8, self.dataset_number_classes)
        else:
            self.linear_1 = nn.Linear(self.final_number_channels * 2 * 2, self.dataset_number_classes)

    def _create_convolutional_layers(self, channel_counts):
        conv_parameters = {"kernel_size": self.kernel_size, "padding": int((self.kernel_size - 1) / 2), "stride": self.stride}
        channels = [int(c) for c in [self.dataset_number_channels] + channel_counts]
        for i in range(self.number_stages):
            #stage_input
            convolutional_input_layer = nn.Conv2d(channels[i], channels[i + 1], kernel_size=self.kernel_size, stride=self.stride, padding=int((self.kernel_size - 1) / 2))
            ...
            #else:
            graphNNs = [GraphNN(graph=graph, settings=self.settings, **conv_parameters) for graph in self.graphs]
            diffNN = DiffNN(settings=self.settings, graphNNs=graphNNs, stage_index=i)
            self.stages.append(diffNN)
    ...
    def _create_pooling_layers(self, base):
        for i in range(self.number_stages):
            if base[i] == 1:
                ...
                pooling_layer = torch.nn.AvgPool2d(base[i], stride=base[i], padding=self.padding[i])
            else:
                pooling_layer = torch.nn.MaxPool2d(base[i], stride=base[i], padding=self.padding[i])
    ...
    def _calculate_pooling(self, final_channel_count, number_stages, number_features):
        ...
        if number_stages < 3:
            ...
            base = [2] + [2 if i in indices else 1 for i in range(number_stages - 2)] + [2]
        # padding
        assert number_features in [28,32], "Implementation only for MNIST and CIFAR10 with corresponding features sizes 28x28 resp. 32x32"
        padding = [0 for i in range(number_stages)]
        if number_features == 28:
            if number_stages == 1:
                padding[0] = 2
            else:
                padding = [1 if functools.reduce(lambda x,y: x*y, base[:i], 1) == 4 and base[i] > 1 else 0 for i in range(number_stages)]

        # channel
        channel_counts = [final_channel_count if i == 0 else final_channel_count / functools.reduce(lambda x,y: x*y, base[-i:], 1) for i in range(number_stages)]
        channel_counts.reverse()
        assert all([math.isclose(round(c), c) for c in channel_counts]), "Channel count not even enough"
