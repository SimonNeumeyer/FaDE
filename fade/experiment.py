...
class Experiment:
    def __init__(self):
        ...
        self.dataset = MNIST()
        self.loader_train, _ = self._get_data_loader(batch_size=257, number_workers=4, ratio=0.9, distinct=True)
        self.loader_test, _ = self._get_data_loader(batch_size=257, number_workers=4, ratio=0.1, distinct=True)
        self.model = ConvModel2()
        self.loss = nn.CrossEntropyLoss()
        self.optimizer = Adam(self.get_parameters(self.model), lr=1)
        self.scheduler = LinearLR(self.optimizer, total_iters=5, start_factor=0.05, end_factor=0.01)
    ...
    def evaluate(self):
        ...
        with torch.no_grad():
            ...
            for i, o in self.loader_test:
                ...
                accuracy = ((batchCount - 1) * accuracy + self._accuracy(i, o)) / batchCount
        self.model.train()
...
class ConvModel(nn.Module):
    def __init__(self):
        ...
        depth = 3
        self.m = nn.Sequential(nn.Conv2d(1, 8, kernel_size=3, stride=1, padding=1), nn.ReLU())
        for i in range(depth):
...
class ConvModel2(nn.Module):
    def __init__(self):
        ...
        for i in "abc":
            c = 1 if i == "a" else 8
            self.m[i] = nn.Sequential(nn.Conv2d(c, 8, kernel_size=3, stride=1, padding=1), nn.ReLU())
        self.pooling = torch.nn.MaxPool2d(4, stride=4, padding=2)
        self.linear1 = nn.Linear(512, 10)
