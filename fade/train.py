...
class Trainer:
    def __init__(self, settings, dataset, model, results, cuda=True):
        ...
        if (
            self.settings[Constants.darts_optimization_method]
            == Constants.darts_optimization_method_trivial
        ):
            self.loader_train, self.batch_count_train_all = self._get_data_loader(
                batch_size=self.settings[Constants.optimization_batch_size],
                number_workers=4,
                ratio=0.9,
                distinct=True,
            )
            ...
            (
                self.loader_train_weights,
                self.batch_count_train_weights,
            ...
            (
                self.loader_train_alphas,
                self.batch_count_train_alphas,
            ) = self._get_data_loader(
                batch_size=self.settings[Constants.optimization_batch_size],
                number_workers=4,
                ratio=0.9,
        ...
        if self.settings[Constants.optimization_optimizer] == Constants.OPTIMIZER_ADAM:
            ...
            self.optimizer_weights = SGD(
                model.get_parameters(alphas=False), lr=1, momentum=0.8
            )
            self.optimizer_alphas = SGD(
                model.get_parameters(alphas=True), lr=1, momentum=0.8
            )
        else:
            raise NotImplementedError(
                f"Optimizer {self.settings[Constants.optimization_optimizer]} not supported"
            )
        if self.settings[Constants.darts_deactivate_darts] == 1:
            ...
            self.optimizer_all = Adam(
                model.get_parameters(alphas=False) + model.get_parameters(alphas=True),
                lr=1,
            )
        self.scheduler_all = LinearLR(
            self.optimizer_all,
            total_iters=min(5, self.settings[Constants.optimization_number_epochs]),
            start_factor=self.settings[Constants.optimization_learning_rate_start],
            end_factor=self.settings[Constants.optimization_learning_rate_end],
        )
        self.scheduler_weights = LinearLR(
            self.optimizer_weights,
            total_iters=min(5, self.settings[Constants.optimization_number_epochs]),
            start_factor=self.settings[Constants.optimization_learning_rate_start],
            end_factor=self.settings[Constants.optimization_learning_rate_end],
        )
        self.scheduler_alphas = LinearLR(
            self.optimizer_alphas,
            total_iters=min(5, self.settings[Constants.optimization_number_epochs]),
            start_factor=self.settings[Constants.darts_learning_rate_start],
            end_factor=self.settings[Constants.darts_learning_rate_end],
        )
    ...
    def _train_epoch(self, epoch, **kwargs):
        ...
        if (
            ...
            number_iterations = max(
                self.batch_count_train_weights, self.batch_count_train_alphas
            )
        for i in range(number_iterations):
            if (
                ...
                self.settings[Constants.darts_optimization_method]
                == Constants.darts_optimization_method_first_order
            ):
                ...
                self.settings[Constants.darts_optimization_method]
                == Constants.darts_optimization_method_second_order
            ):
    ...
    def _train_batch_second_order(self):
        ...
        train_input, train_target = train_input.to(self.device), train_target.to(
            self.device
        )
        valid_input, valid_target = next(self.loader_train_alphas)
        valid_input, valid_target = valid_input.to(self.device), valid_target.to(
            self.device
        )
        ...
        g_w_t = [
            g_.data.clone()
            for g_ in torch.autograd.grad(valid_loss, p_convs, retain_graph=True)
        ]
    ...
    def _evaluate(self, epoch):
        ...
        with torch.no_grad():
            ...
            for i, o in self.loader_test:
                ...
                accuracy = (
                    (batchCount - 1) * accuracy + self.accuracy(i, o)
                ) / batchCount
        self.model.train()
